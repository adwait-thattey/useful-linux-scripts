...
then
    ...
    PS1="$YELLOW$CURRHOST$WHITE [$BOLD$BLUE$(date +%b-%d)$WHITE@$BLUE\t $USER_PROMPT_COLOR$USER $ORANGE \W$WHITE] $YELLOW\$$RESET_COLOR "
...
if [[ $CURRHOST == "uslinux01" ]]; then
    source ~/adwait/adwait_script.sh
else
    set_custom_bash_prompt
fi
